        ...
        <q-item-section>
          <q-item-label>
            <template v-if="event.type === 'GOAL'">
              ...
              Goal: {{ event.player?.firstName }} {{ event.player?.lastName }}
            ...
            <template v-else-if="event.type === 'YELLOW_CARD'">
              ...
              Yellow Card: {{ event.player?.firstName }} {{ event.player?.lastName }}
            </template>
            <template v-else-if="event.type === 'RED_CARD'">
              <q-icon name="square" color="negative" size="xs" class="q-mr-xs" />
              Red Card: {{ event.player?.firstName }} {{ event.player?.lastName }}
            </template>
            <template v-else-if="event.type === 'SUBSTITUTION'">
              <q-icon name="swap_vert" color="blue" size="xs" class="q-mr-xs" />
              Sub: {{ event.player?.firstName }} {{ event.player?.lastName }} for {{ event.secondaryPlayer?.firstName }} {{ event.secondaryPlayer?.lastName }}
            </template>
          </q-item-label>
        </q-item-section>
      </q-item>
